# cloudwatch alarm, to scale-out for more than 80% CPU utilization
resource "aws_cloudwatch_metric_alarm" "high-cpu-jenkins-workers-alarm" {
  ...
  namespace           = "AWS/EC2"
  period              = "120" # for 2 minutes, greater or equal to 80%
  statistic           = "Average" # aggregated metrics of all the worker instances.
  threshold           = "80"
